...
class Stream extends DataView {
  ...
  /**@protected */
  get __offset__() {
    return this.__off_set__ ?? 0;
  }
  set __offset__(v) {
    if (v > this.byteLength)
      throw new EndOfStreamError('Offset cant be out of Stream.');
    this.__off_set__ = v;
    if (v > this.__size__) this.__size__ = v;
  }
  /**@readonly */
  get offset() {
  ...
  /**@readonly */
  get EndOfStream() {
    return this.__offset__ >= this.byteLength;
  }
  setOffset(num) {
    if (num > this.byteLength)
      throw new EndOfStreamError('Offset cant be out of Stream.');
  }
  static fromString(text, options, ...params) {
    const { bufferLength = 32e3, base = 16 } = options ?? {};
    const BYTES_PER_ELEMENT = base / 8,
      property = 'setUint' + base;
    if (text.length * BYTES_PER_ELEMENT > bufferLength)
      throw new RangeError(
        "Can't fit text to specified buffer, please increase buffer length"
      );
    ...
    for (let i = 0; i < text.length; i++)
      typedArray[property](
        i * BYTES_PER_ELEMENT,
        String.prototype.charCodeAt.call(text, i) - 1
      );
  ...
  static toString(stream, base = 16) {
    const BYTES_PER_ELEMENT = base / 8,
      property = 'getUint' + base;
    const codes = [];
    if (!(property in stream)) throw new Error('Invalid base: ' + base);
    for (let i = 0; i < stream.size; i += BYTES_PER_ELEMENT)
      codes.push(1 + stream[property](i));
...
class BinaryStreamWriter extends Stream {
  ...
  writeString(text, base = 8) {
    ...
    let out = 0;
    for (let i = 0; i < text.length; i++)
      out += this[property](String.prototype.charCodeAt.call(text, i));
    return out;
  }
  /**@param {number} num */
  writeByte(num) {
  ...
  /**@param {number} num */
  writeUint8(num) {
    if (this.EndOfStream || this.__offset__ + 1 > this.byteLength)
      throw new EndOfStreamError('You cant write at end of the stream');
  ...
  /**@param {number} num */
  writeInt16(num) {
    if (this.EndOfStream || this.__offset__ + 2 > this.byteLength)
      throw new EndOfStreamError('You cant write at end of the stream');
  ...
  /**@param {number} num */
  writeInt32(num) {
    if (this.EndOfStream || this.__offset__ + 4 > this.byteLength)
      throw new EndOfStreamError('You cant write at end of the stream');
  ...
  /**@param {number} num */
  writeFloat64(num) {
    if (this.EndOfStream || this.__offset__ + 8 > this.byteLength)
      throw new EndOfStreamError('You cant write at end of the stream');
...
class BinaryStreamReader extends Stream {
  ...
  readBytes(length = 0) {
    if (length + this.offset > this.byteLength)
      length = this.byteLength - this.offset;
  ...
  readString(length = 0, base = 8) {
    const BYTES_PER_ELEMENT = base / 8,
      property = 'readUint' + base;
    if (length * BYTES_PER_ELEMENT + this.offset > this.byteLength)
      length = Math.floor((this.byteLength - this.offset) / BYTES_PER_ELEMENT);
    if (!(property in this)) throw new Error('Invalid base: ' + base);
    let data = [];
  ...
  readUint8() {
    if (this.EndOfStream || this.__offset__ + 1 > this.byteLength)
      throw new EndOfStreamError('You cant read at end of the stream');
...
class NBTStreamWriter extends BinaryStreamWriter {
  constructor(stream, options = new NBTWriterOptions()) {
    super(stream);
    if (!(options instanceof NBTWriterOptions))
      Object.setPrototypeOf(options, NBTWriterOptions.prototype);
  ...
  writeTypedArray(array) {
    ...
    for (const data of array)
      bytes += this.__options__.writers[typeOf](this, data);
    return bytes;
  }
  writeBoolean(bool) {
  ...
  writeArray(array) {
    ...
    for (const data of array) {
      ...
      if (!type) continue;
      bytes +=
        this.writeType(type) + this.__options__.writers[type](this, data);
    }
    return bytes;
  }
  writeCompoud(object) {
  ...
  writeType(type) {
    return this.writeByte(
      typeof type === 'string'
        ? (this.__options__.nbtTypes[type] ?? 0)
        : (type ?? 0)
    );
  }
}
class NBTStreamReader extends BinaryStreamReader {
  constructor(stream, options = new NBTReaderOptions()) {
    super(stream);
    if (!(options instanceof NBTReaderOptions))
      Object.setPrototypeOf(options, NBTReaderOptions.prototype);
    this.__options__ = options;
  }
  /**@readonly @private @type {NBTReaderOptions} */
  __options__;
  readTypedArray() {
    const typeOf = this.readType(),
      array = [];
    let length = this.readUint16();
    for (let i = 0; i < length; i++)
      array.push(this.__options__.readers[typeOf](this));
    return array;
  }
  readBoolean() {
...
class NBT {
  ...
  static stringify(object, options) {
    let o = new NBTStreamWriter(32e3, options ?? new NBTWriterOptions());
    NBT.WriteNBT(object, o);
    return o.toString();
  }
  /**@param {unknown} object @param {NBTReaderOptions?} options */
  static parse(string, options) {
    let stream = NBTStreamReader.fromString(
      string,
      null,
      options ?? new NBTReaderOptions()
    );
    return NBT.ReadNBT(stream);
  }
  static createNewWriters(object) {
    return object
      ? Object.setPrototypeOf(object, defaultWriters)
      : Object.create(defaultWriters);
  }
  static createNewReaders(object) {
    return object
      ? Object.setPrototypeOf(object, defualtReaders)
      : Object.create(defualtReaders);
  }
}
...
function getScoreboard() {
  if (!_scoreboard_) _scoreboard_ = world.scoreboard;
  return _scoreboard_;
}
...
class ScoreboardDatabaseManager extends Map {
  ...
  constructor(
    objective,
    saveMode = DatabaseSavingModes.EndTickSave,
    interval = 5
  ) {
    super();
    ...
    if (!objective)
      throw new RangeError('Firt parameter si not valid: ' + objective);
    if (
      typeof objective !== 'string' &&
      !(objective instanceof ScoreboardObjective)
    )
      throw new RangeError('Firt parameter si not valid: ' + objective);
  ...
  set(key, value) {
    ...
    if (newValue.length > this.maxLength)
      throw new RangeError('Value is too large for one property');
    super.set(key, value);
  ...
  clear() {
    ...
    super.clear();
    this._source_.clear();
    getScoreboard().removeObjective(this.objective);
    this._scoreboard_ = getScoreboard().addObjective(
      this._nameId_,
      this._nameId_
    );
  }
  ...
  rebuild() {
    ...
    const newScores = getScoreboard().addObjective(
      this._nameId_,
      this._nameId_
    );
